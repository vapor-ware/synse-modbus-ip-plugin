...
// Sdk ticket: https://github.com/vapor-ware/synse-sdk/issues/474
...
func verifyReadContexts(t *testing.T, expected []*sdk.ReadContext, actual []*sdk.ReadContext) {
	...
	for i := 0; i < expectedLen; i++ {
		...
		// Info was removed from the reading in synse v3, but it should be put back.
		// Sdk ticket: https://github.com/vapor-ware/synse-sdk/issues/475
...
// FUTURE: Need to add 6 e-gauge devices.
// FUTURE: Need to add the carousel controller.
// FUTURE: This is the old VEM register set, circa January 2019.
...
func TestVEM(t *testing.T) {
	...
	registerDevices := []*sdk.Device{
		...
		&sdk.Device{
			...
			Output:  "gallonsPerMin", // Consider spelling out minute (sdk).
			Handler: "holding_register",
		},
	...
	expectedRegisterReadContexts := []*sdk.ReadContext{
		...
		&sdk.ReadContext{
			...
			Reading: []*output.Reading{
				{
					// Confusing is the output string called output is percentage when the reading's output string is percent.
